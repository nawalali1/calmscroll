  ...
  return (
    <nav aria-label="Primary navigation" className="pointer-events-none fixed inset-x-0 bottom-0 z-50 flex justify-center">
      <div className="pointer-events-auto w-full max-w-[420px] px-4 pb-[calc(env(safe-area-inset-bottom)+0.75rem)]">
        <div className="flex items-center justify-between gap-2 rounded-3xl border border-[var(--card-border)] bg-white/95 px-3 py-2 shadow-md">
          {tabs.map(({ href, label, Icon }, index) => {
            ...
            return (
              <Link
                ...
                href={href}
                className={cn(
                  "flex min-h-[52px] flex-1 flex-col items-center justify-center rounded-2xl px-2 text-xs font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-2 focus-visible:ring-offset-white",
                  active ? "bg-white text-[var(--ink)] shadow-sm" : "text-[var(--ink-muted)] hover:bg-white/80"
                )}
